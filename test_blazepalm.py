...
palmnet   = Palm(prob_threshold=0.7, nms_threshold=0.3, level="light", use_gpu=False)
...
while (not stop):
    ...
    if (current_time - last_time) > compute_interval : 
        ...
        palms, times_palms = palmnet(frame, scale=True)
        toc = time.perf_counter(); 

        cnn_time += toc-tic
        cnn_count += 1
        cnn_routines_time += times_palms
        
        tic = time.perf_counter()
        for palm in palms: 
            palm.draw(display_img)
    ...
    if (current_time - last_update_time) > update_interval:
        ...
        if cnn_count > 0: 
            logger.log(logging.INFO, "Find Palm       {:.2f} ms".format(1000.*cnn_time/cnn_count))
            ...
            logger.log(logging.INFO, "  Create Palm   {:.3f} ms".format(1000.*cnn_routines_time[4]/cnn_count))
