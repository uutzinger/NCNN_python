...
from blazepersonpose import PersonLandmarkDetect
...
skeletonnet = PersonLandmarkDetect(prob_threshold=0.5, nms_threshold=0.3, vis_threshold=0.1, level="heavy", use_gpu=False)
...
while (not stop):
    ...
    if (current_time - last_time) > compute_interval : 
        ...
        for (person, portrait) in zip(persons, portraits):
            ...
            frame_roi, mat_trans, rotatedObject = extractObjectROI(frame, person, target_size = 256, simple=True)
            person_pose, times_sekleton =  skeletonnet(frame_roi)

            cnn_skeleton_time += times_sekleton
            cnn_skeleton_count += 1

            if person_pose is not None:
                tic = time.perf_counter()
                person_pose.invtransform(mat_trans)
                person_pose.draw(display_img)
                portrait.drawRect(display_img, color=(0,255,0))
    ...
    if (current_time - last_update_time) > update_interval:
        ...
        if cnn_count > 0: 
            ...
            logger.log(logging.INFO, "  Create Person {:.2f} ms".format(1000.*cnn_routines_time[4]/cnn_count))

        if cnn_skeleton_count > 0: 
            logger.log(logging.INFO, "Skeleton")
            ...
            logger.log(logging.INFO, "  Create        {:.2f} ms".format(1000.*cnn_skeleton_time[2]/cnn_skeleton_count))
