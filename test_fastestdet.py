...
from utils_yolo import drawYoloObjects
...
# -----------------------------------------------------------------------------
# Object CNN
net = FastestDet(prob_threshold=0.6, nms_threshold=0.5, use_gpu=True)
...
while (not stop):
    ...
    if (current_time - last_time) > compute_interval : 
        ...
        objects, times = net(frame, scale=True, use_weighted_nms=False)
        ...
        tic = time.perf_counter()
        drawYoloObjects(display_img, objects, (255,255,255))
    ...
    if (current_time - last_update_time) > update_interval:
        ...
        if ff_count > 0: 
            ...
            logger.log(logging.INFO, "  Extract       {:.2f} ms".format(1000.*ss_time[1]/ff_count))
            logger.log(logging.INFO, "  Generation    {:.2f} ms".format(1000.*ss_time[2]/ff_count))
            logger.log(logging.INFO, "  Select NMS    {:.3f} ms".format(1000.*ss_time[3]/ff_count))
            logger.log(logging.INFO, "  Rotate        {:.3f} ms".format(1000.*ss_time[4]/ff_count))
            logger.log(logging.INFO, "  Create Obj    {:.3f} ms".format(1000.*ss_time[5]/ff_count))
